...
后台视觉使用仓库内 `docs/ai-design-reference/sneat-bootstrap-free/` 的 Sneat Bootstrap FREE 参考：浅灰蓝画布、白色浮层卡片、固定侧边菜单、顶部导航、紫色主操作色和 Bootstrap 栅格密度。平台首页只展示认证、授权和运行状态；业务页面必须通过业务模块注册协议接入。实现样式集中在 `src/style.css`，不加载第三方 Bootstrap CSS。
