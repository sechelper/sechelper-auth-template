...
概览页将应用信息与系统状态、依赖服务合并在 APPLICATION 卡片中，直接展示运行时长、最近检查时间及各依赖的独立健康状态，不显示依赖健康数量汇总；服务器资源中的磁盘和 Goroutines 使用并列紧凑卡片展示数值、趋势与必要容量信息，并在窄屏下改为单列；访问控制诊断和 Manifest 状态继续集中展示。所有展示的读取数据来自平台接口。平台 Dashboard 可以跳转到业务模块，但不能读取或聚合业务模块数据库，也不能展示订单数量、订单金额、最近订单等业务指标。
