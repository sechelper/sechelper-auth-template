...
| 资源级授权与租户隔离 | 部分实现 | `api/internal/modules/authorization/`、`api/internal/modules/orders/` | 授权资源单元测试通过 | Orders 当前为只读且未实现租户/所有权过滤 | 将授权下沉到订单 Use Case，补越权测试 |
...
| 前台/后台 Sneat 风格 UI | 部分实现 | `web/`、`web/admin/`、`docs/ai-design-reference/` | 两端独立 Vite 生产构建通过 | 浏览器响应式、键盘和 WCAG 未在本次本机运行 | 执行 Playwright 桌面/平板/移动矩阵 |
