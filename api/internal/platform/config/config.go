...
type BootstrapConfig struct {
	...
	InstallLockFile string `mapstructure:"installLockFile"`
...
func Load(args ...string) (Config, error) {
	...
	setDefaults(v)
	dotenv, err := readDotEnv(filepath.Join(filepath.Dir(path), ".env"))
	if err != nil {
		return Config{}, err
	}
	if err := applySecretEnvironmentOverrides(v, dotenv); err != nil {
		return Config{}, err
	}
	var c Config
	...
	if strings.TrimSpace(c.Bootstrap.InstallKey) != "" {
		if strings.TrimSpace(c.Bootstrap.DatabaseURL) == "" || strings.TrimSpace(c.Bootstrap.EncryptionKey) == "" || strings.TrimSpace(c.Bootstrap.InstallKey) == "" {
			return Config{}, errors.New("install mode requires bootstrap.databaseUrl, bootstrap.encryptionKey, and bootstrap.installKey")
		}
		return c, nil
	}
...
// ApplyConfigurationValues overlays the framework-supported configuration
// center keys. Bootstrap values, including the database connection, are
// deliberately excluded because the configuration center depends on them.
func ApplyConfigurationValues(base Config, values map[string]string) (Config, error) {
	if base.Bootstrap.InstallMode && len(values) == 0 {
		return base, nil
	}
...
func readDotEnv(path string) (map[string]string, error) {
	dotenv := viper.New()
	dotenv.SetConfigFile(path)
	dotenv.SetConfigType("dotenv")
	...
	for key, value := range dotenv.AllSettings() {
		values[strings.ToUpper(key)] = fmt.Sprint(value)
	}
	return values, nil
}

func environmentValue(name string, dotenv map[string]string) (string, bool) {
...
func applySecretEnvironmentOverrides(v *viper.Viper, dotenv map[string]string) error {
	if value, ok := environmentValue("DATABASE_URL", dotenv); ok {
	...
	if value, ok := environmentValue("REDIS_URL", dotenv); ok {
		v.Set("app.redisUrl", strings.TrimSpace(value))
	}
	if value, ok := environmentValue("CONFIG_CENTER_ENCRYPTION_KEY", dotenv); ok {
		v.Set("bootstrap.encryptionKey", strings.TrimSpace(value))
	}
	if value, ok := environmentValue("CONFIG_CENTER_INSTALL_KEY", dotenv); ok {
		v.Set("bootstrap.installKey", strings.TrimSpace(value))
	}
	return nil
}
...
func configurationBindings() map[string]string {
	return map[string]string{
		"app.name": "APP_NAME", "app.environment": "APP_ENV", "app.listenAddr": "LISTEN_ADDR", "app.publicWebOrigin": "PUBLIC_WEB_ORIGIN", "app.apiOrigin": "API_ORIGIN", "app.primaryDomain": "PRIMARY_DOMAIN", "app.testDomainSuffix": "TEST_DOMAIN_SUFFIX",
		"server.readTimeout": "SERVER_READ_TIMEOUT", "server.writeTimeout": "SERVER_WRITE_TIMEOUT", "server.idleTimeout": "SERVER_IDLE_TIMEOUT", "server.shutdownTimeout": "SERVER_SHUTDOWN_TIMEOUT",
