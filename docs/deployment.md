...
管理后台概览页通过框架 API `GET /v1/admin/deployment/guide` 展示部署版本信息、Bootstrap 配置、配置中心和框架检查状态，只需要 `admin:access`。后台“配置中心”页面负责框架环境变量填写和保存；旧地址 `/admin/deployment` 继续作为兼容地址。接口只返回应用版本、Build ID、bootstrap 是否已配置、配置中心是否在启动阶段加载、数据库连通性、配置中心表和迁移状态，不返回数据库 URL、密码、Token、Session 密钥或其他敏感值。
