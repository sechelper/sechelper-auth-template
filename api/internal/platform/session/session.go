...
func (s *PostgresStore) Update(ctx context.Context, value Session) error {
	...
	result, err := s.db.ExecContext(ctx, `UPDATE framework.authentication_sessions SET email=$2, permissions=$3, refresh_token_ciphertext=$4, id_token_ciphertext=$5, expires_at=$6, version=version+1, updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND version=$7 AND revoked_at IS NULL`, value.ID, value.Email, permissions, value.RefreshTokenCiphertext, value.IDTokenCiphertext, value.ExpiresAt, value.Version)
	if err != nil {
		return err
	}
...
func (s *PostgresStore) RotateRefreshToken(ctx context.Context, id string, rotate func(context.Context, Session) (Session, error)) (Session, error) {
	...
	if err != nil {
		if errors.Is(err, ErrRefreshReuse) {
			if _, revokeErr := tx.ExecContext(ctx, `UPDATE framework.authentication_sessions SET revoked_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND revoked_at IS NULL`, id); revokeErr != nil {
	...
	result, err := tx.ExecContext(ctx, `UPDATE framework.authentication_sessions SET email=$2, permissions=$3, refresh_token_ciphertext=$4, id_token_ciphertext=$5, expires_at=$6, version=version+1, updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND version=$7 AND revoked_at IS NULL`, value.ID, value.Email, permissions, value.RefreshTokenCiphertext, value.IDTokenCiphertext, value.ExpiresAt, current.Version)
	if err != nil {
		return Session{}, err
	}
	count, err := result.RowsAffected()
...
func (s *PostgresStore) Revoke(ctx context.Context, id string) error {
	result, err := s.db.ExecContext(ctx, `UPDATE framework.authentication_sessions SET revoked_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE id=$1 AND revoked_at IS NULL`, id)
	if err != nil {
		return err
	}
	count, err := result.RowsAffected()
