...
export function OrdersPage() {
  ...
  const load = () => request("/v1/orders?page%5Bsize%5D=100").then((value) => setData(value.data || [])).catch(setError);
  useEffect(() => { load(); }, []);
  ...
  return <>
    ...
    <section className="card order-list-card">
      ...
      {filtered.length === 0 ? <div className="empty"><strong>{data.length ? '没有匹配订单' : '暂无订单'}</strong><span>{data.length ? '请调整搜索条件或状态筛选。' : '当前没有可展示的订单记录。'}</span></div> : <div className="table-wrap"><table><thead><tr><th>订单编号</th><th>创建时间</th><th>客户</th><th>状态</th><th>金额</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td className="mono">{item.id}</td><td>{item.createdAt}</td><td>—</td><td><span className="tag">{item.status}</span></td><td>{(item.totalMinor / 100).toFixed(2)} {item.currency}</td></tr>)}</tbody></table></div>}
    </section>
    <style>{`
