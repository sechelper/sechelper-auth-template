...
const releasePath = path.resolve(process.argv[2] ?? "project.yaml");
...
try {
  ...
  const matches = [...original.matchAll(/^  buildId:\s*(\d{14})\s*$/gm)];
  if (matches.length !== 1) throw new Error("project.yaml must contain exactly one release.buildId before allocation");
  ...
  const updated = original.replace(/^  buildId:\s*\d{14}\s*$/m, `  buildId: ${id}`);
