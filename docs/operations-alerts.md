...
API 的 `/metrics` 暴露认证计数器，生产环境必须通过 `Authorization: Bearer $METRICS_TOKEN` 访问。生产 Prometheus 应通过安全的 Secret 注入该 Header，并加载 [`deploy/prometheus/auth-template-alerts.yml`](../deploy/prometheus/auth-template-alerts.yml)。
