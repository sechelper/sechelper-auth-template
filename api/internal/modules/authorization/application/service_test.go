...
func TestServiceRequiresPermission(t *testing.T) {
	...
	if err := store.Create(context.Background(), session.Session{ID: "session-1", Subject: "user-1", ApplicationCode: "demo", Permissions: []string{"resource:read"}, ExpiresAt: time.Now().Add(time.Hour)}); err != nil {
		t.Fatal(err)
	}
	service := NewService(store, NewMemoryCache())
	if _, err := service.Require(context.Background(), "session-1", "resource:read"); err != nil {
		t.Fatal(err)
	}
	if _, err := service.Require(context.Background(), "session-1", "resource:approve"); err != ErrForbidden {
...
func TestServiceRejectsRevokedSessionEvenWhenCached(t *testing.T) {
	store := session.NewMemoryStore()
	if err := store.Create(context.Background(), session.Session{ID: "session-2", Subject: "user-2", ApplicationCode: "demo", Permissions: []string{"resource:read"}, ExpiresAt: time.Now().Add(time.Hour)}); err != nil {
		t.Fatal(err)
	}
	service := NewService(store, NewMemoryCache())
