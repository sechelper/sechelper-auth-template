...
import { menu, navigate, normalizePathname } from "./navigation.jsx";
...
function Page({ pathname, session }) { const businessRoute = adminBusinessRoutes().find((route) => route.path === pathname); if (businessRoute) { const BusinessPage = businessRoute.element; return <BusinessPage />; } if (pathname === "/admin") return <DashboardPage session={session} />; if (pathname === "/admin/manifest") return <ManifestPage />; if (pathname === "/admin/sessions") return <SessionsPage />; if (pathname === "/admin/permissions") return <PermissionsPage />; if (pathname === "/admin/audit-events") return <AuditPage />; if (pathname === "/admin/resources") return <ResourcesPage />; return <NotFoundPage />; }
...
export function AdminApp() {
  ...
  const visibleMenu = [...menu, ...adminBusinessNavigation()].filter((item) => !item.permission || hasPermission(item.permission));
  return <AdminLayout pathname={pathname} menu={visibleMenu} state={state} onRefresh={refreshSession} onLogout={async () => { setSignedOut(true); await logout(); window.location.assign("/"); }}><Page pathname={pathname} session={state} /></AdminLayout>;
}
